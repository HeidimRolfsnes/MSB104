...
we will now look at growth and inequality in the countries at Nuts2 level. We will also include more variables to see if there are other conditions that affect growth in GDP per capita.
...
In @fig-10 we can see how the Gini is distributed per country.
France has one point that stands out from the others, while the other
most countries have their scores between 0 and 0.1. After we divided the countries into Nuts2 regions, we see that the countries, apart from France, do not have that many observations. we therefore choose to only look at all the countries together or France in further tests
...
In this regresion we will look at the impact regional development (X) has on regional inequality (Y). This means how much an
increase in X will have on a increase in Y. We use Gini as Y and GDP per capita as X.
...
```
In this model, we see that France has 22 observations and an R2 of 81.4%, which means that a large part of the increase in GINI has a connection with an increase in GDP per capita. When we look at all the countries together, we see that we have 38 observations and that R2 is only 12.2%. which means that there is less correlation between an increase in Gini and an increase in Gdp per capita
...
The model above shows a negative Gini compared to when we look at all the countries together, and we therefore choose to remove Paris from the data set to see if this has any effect on the model.
...
```
In this plot we can se a change
...
normally distributed [@Wooldridge]. we can make a hypotesis by using
this formula.
...
#Gen Table
  huxreg(
  list( "Nuts2" = plm, "year" = plm, "All" = plm ),
  statistics = c(N = "nobs", R2 = "r.squared"),
  stars = c("*" = 0.1, "**" = 0.05, "***" =0.01),
  note = "Note): {stars} T statistics in brackets."
  )
