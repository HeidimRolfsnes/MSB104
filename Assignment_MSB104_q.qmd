...
#Gen Table
  ...
  # Did we get the log right on lm.25?  
```


```{r}
gdppop2010 <- gdppop2010
lm.26 = lm(Gini2 ~ log(gdp_per_capita), filter(gdppop2010, Fra==1))%>%
coeftest(vcov = hccm, save = TRUE)
lm.27 = lm(Gini2 ~ log(I(gdp_per_capita)^2), filter(gdppop2010, Fra==1))%>%
  coeftest(vcov = hccm, save = TRUE)
lm.28 = lm(Gini2 ~ log(I(gdp_per_capita)^3), filter(gdppop2010, Fra==1))%>%
  coeftest(vcov = hccm, save = TRUE)
...
```
lm.29 = lm(Gini2 ~ log(gdp_per_capita)*Fra, gdppop2010)%>%
  coeftest(vcov = hccm, save = TRUE)
  , "Interaction" = lm.29
...
#plot
reg %>% 
  ggplot(aes(x = fitted(reg), y = resid(reg))) +
...
Eks fra oppgave 4 i R. BP = 352.89, df = 15, p-value < 2.2e-16
...
With a p-value = 0,6720791 we can reject $H_0$ (homoskedasticity). Hence we facing Heteroskedasticity!
...
Alder har noe å si om befolkningen er ung eller gammel med tanke på utvikling av region/land. 
