...
gdppop2010a <- gdppop2010 %>%
  filter(nuts2 != 'FR') 
#removing Paris from gdppop2010
#Sjekk med Andre om å fjerne Portugal og Paris. 
```
...
lm.25 = lm(Gini2 ~ gdp_per_capita*Fra, gdppop2010)

#Gen Table
  huxreg(
  list("Only France (log)" = lm.23, "No France (log)" = lm.24, "Interaction" = lm.25),
  statistics = c("N" = "nobs", 
      R2 = "r.squared", "F statistic" = "statistic",
      "P value" = "p.value"),
   stars = c("*" = 0.1, "**" = 0.05, "***" =0.01),
...
lm.28 = lm(Gini2 ~ log(I(gdp_per_capita)^3), filter(gdppop2010, Fra==1))%>%
  coeftest(vcov = hccm, save = TRUE)
lm.29 = lm(Gini2 ~ gdp_per_capita*Fra, gdppop2010)%>%
  coeftest(vcov = hccm, save = TRUE)

#Gen Table
  huxreg(
  list("Only France (log)" = lm.26, "Only France (log) ^2" = lm.27, "Only France ^3" = lm.28, "Interaction" = lm.29),
  statistics = c("N" = "nobs", 
      R2 = "r.squared", "F statistic" = "statistic",
      "P value" = "p.value"),
   stars = c("*" = 0.1, "**" = 0.05, "***" =0.01),
...
Sjekk med Paris - Legg inn to regresjonslinjer 

```{r}
ggplot(data = gdppop2010, aes(x = Gini2, y = gdp_per_capita)) +
  geom_point() +
...
```{r
ggplot(data = mlrdata, aes(x = Gini2, y = gdp_per_capita)) +
geom_point(size = 2.5, alpha = 0.8, aes(color = Fra, shape = Fra)) +
...
data('gdppop2010')
#Estimate 
reg<-lm(Gini2 ~ gdp_per_capita + Edu + lea + Road, data = gdppop2010) 
#plot
reg %>% 
...
```{r,  echo=TRUE, include=TRUE}
#Estimate 
reg<-lm(Gini2 ~ gdp_per_capita + Edu + lea, data = gdppop2010)
# Breusch-Pagan test F-Test (LM estimator)
bptest(reg)$p.value

```

With a p-value = 0,07012004 we can reject $H_0$ (homoskedasticity). Hence we facing Heteroskedasticity!
With a p-value = 0,03452553 we can reject $H_0$ (homoskedasticity). Hence we facing Heteroskedasticity!

*Log-log estimate*

```{r,  echo=TRUE, include=TRUE}
#Estimate 
reg <- lm(log(Gini2) ~ log(gdp_per_capita) + log(Edu) + log(lea), data = gdppop2010) 
# Breusch-Pagan test F-Test (LM estimator)
bptest(reg)$p.value

```

With a p-value = 0,01209977 we can not reject $H_0$ (homoskedasticity).
With a p-value = 0,004737669 we can not reject $H_0$ (homoskedasticity).
...
#### Graph 1 {data-link="Graph 1"}

```{r Gdp_in_Denmark, fig.cap= "GDP in Denmark", label="Gdp_in_Denmark"}
#| echo = FALSE
gdppop%>%
