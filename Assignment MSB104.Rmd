...
```{r NUTS2 in Denmark}
#| echo = FALSE
  ggplot(DK, aes(x = Year, y=gini_n2, fill=id_nuts2, color=id_nuts2)) +
  geom_point(lwd = .8) +
   labs(x = "Year", y = "Gini")
...
#| echo = FALSE
  ggplot(FR1, aes(x = Year, y=gini_n2, fill=id_nuts2, color=id_nuts2)) +
  geom_point(lwd = .8) +
   labs(x = "Year", y = "Gini")
```
...
#| echo = FALSE
  ggplot(HU, aes(x = Year, y=gini_n2, fill=id_nuts2, color=id_nuts2)) +
  geom_point(lwd = .8) +
   labs(x = "Year", y = "Gini")
```
...
```{r}
#| echo = FALSE
  ggplot(SK, aes(x = Year, y=gini_n2, fill=id_nuts2, color=id_nuts2)) +
  geom_point(lwd = .8) +
   labs(x = "Year", y = "Gini")
```
...
Education <- Education %>%
  ...
  mutate(Edu = OBS_VALUE, Year=TIME_PERIOD, id_nuts2=geo) %>%
  select(id_nuts2, Year, Edu)
```

```{r Data set Motorways}
#| echo = FALSE
Transport <- read_csv('transport.csv',show_col_types = FALSE )%>%
  mutate(Road=OBS_VALUE, Year=TIME_PERIOD, id_nuts2=geo) %>%
  select(id_nuts2, Year, Road)
```

```{r Data set life expectancy age }
#| echo = FALSE
Lifeexp <- read_csv('Life.csv' ,show_col_types = FALSE) %>%
mutate(lea = OBS_VALUE, Year = TIME_PERIOD, id_nuts2=geo) %>%
  select(id_nuts2, Year, lea)
```
...
BNP3 <- BNP2 %>%
  mutate(id_nuts2 = substr(Regio_id,1,4))%>%
  mutate(Gini = gini.wtd(Per_capita, w = Population)) %>%
select(id_nuts2, Year, Population, GDP, Per_capita, Gini)
# vi har mange dobble nuts2 regioner, hvordan løse dette?
```
...
BNP2010 <- BNP3 %>%
  filter(Year == 2010)%>%
left_join(Education, by = c("id_nuts2", "Year"))%>%
left_join(Lifeexp, by = c("id_nuts2", "Year"))%>%
left_join(Transport, by = c("id_nuts2", "Year"))%>%
  #economic development = log(Per_capita)
mutate (log_bnppc=log(Per_capita)) %>%
  mutate(Nuts0 = substr(id_nuts2, 1,2))
```
...
Hungary2010 <- BNP2010 %>%
  filter(Nuts0=="HU")
Portugal2010 <- BNP2010 %>%
  filter(Nuts0=="PT")
Slovakia2010 <- BNP2010 %>%
  filter(Nuts0=="SK")
```
...
```{r}
#| echo = FALSE
# create a function to calculate lmr 
lm_function_plot <- function(lmdata){
  lm_bnpcap <- lm(Gini ~ log_bnppc, data = lmdata)
 bnpcap1 <- lm_bnpcap$coefficients[1]
  bnpcap2 <- lm_bnpcap$coefficients[2]
 
   # create plot 
  lmdata %>%
    ggplot(aes(x =log_bnppc, y=Gini, fill=id_nuts2, color=id_nuts2, label=id_nuts2)) +
    geom_point(lwd = .8) +
    geom_text(hjust=-0.3, vjust=0, check_overlap = FALSE) +
    geom_abline(intercept = bnpcap1, slope = bnpcap2, color="yellow", size=0.8) + 
    labs(x = "log(GDP per capita)", y = "Gini")
}
# create a function to calculate lmr2
lm_residuals_plot <- function(lmdata){
  lm_bnpcap <- lm(Gini ~ log_bnppc, data = lmdata)
  bnpcap1 <- lm_bnpcap$coefficients[1]
  bnpcap2 <- lm_bnpcap$coefficients[2]
  lmdata$predicted <- predict(lm_bnpcap)
  lmdata$residuals <- residuals(lm_bnpcap)
  
  # create plot2
  lmdata %>%
    ggplot(aes(x =log_bnppc, y=Gini, fill=id_nuts2, color=id_nuts2)) +
    geom_point(lwd = .8) +
    geom_segment(aes(xend=log_bnppc,yend=predicted)) +
    geom_abline(intercept = bnpcap1, slope = bnpcap2, color="yellow", size=0.8) + 
    labs(x = "log(GDP per capita)", y = "Gini")
}
```
...
```{r lm all}
lm.1 = lm(Gini ~ log_bnppc, data = Denmark2010)
lm.2 = lm(Gini ~ log_bnppc, data = France2010)
lm.3 = lm(Gini ~ log_bnppc, data = Hungary2010)
lm.4 = lm(Gini ~ log_bnppc, data = Portugal2010)
lm.5 = lm(Gini ~ log_bnppc, data = Slovakia2010)
lm.6 = lm(Gini ~ log_bnppc, data = BNP2010)

#Gen Table
  huxreg(
  list("Denmark" = lm.1, "France" = lm.2, "Hungary" = lm.3, "Portugal" = lm.4, "Slovakia" = lm.5, "Total" = lm.6),
  statistics = c(N = "nobs", R2 = "r.squared"),
  coefs = c(log_bnppc = "log_bnppc", const. = "(Intercept)"),
  note = "Note: {stars} T statistics in brackets."
  )
```

```{r}
#| label: fig-lm-Denmark
#| fig-cap: Estimate for Denmark
lm_function_plot(Denmark2010)
```

```{r}
#| label: fig-estimate-France
#| fig-cap: Estimate for France
lm_function_plot(France2010)
```

```{r}
#| label: fig-estimate-Hungary
#| fig-cap: Estimate for Hungary
lm_function_plot(Hungary2010)
```

```{r}
#| label: fig-estimate-Portugal
#| fig-cap: Estimate for Portugal
lm_function_plot(Portugal2010)
```

```{r}
#| label: fig-estimate-Slovakia
#| fig-cap: Estimate for Slovakia
lm_function_plot(Slovakia2010)
```

```{r}
#| label: fig-residuals-Denmark
#| fig-cap: Residuals Denmark
lm_residuals_plot(Denmark2010)
```

```{r}
#| label: fig-residuals-France
#| fig-cap: Residuals France
lm_residuals_plot(France2010)
```

```{r}
#| label: fig-residuals-Hungary
#| fig-cap: Residuals Hungary
lm_residuals_plot(Hungary2010)
```

```{r}
#| label: fig-residuals-Portugal
#| fig-cap: Residuals Portugal 
lm_residuals_plot(Portugal2010)
```

```{r}
#| label: fig-residuals-Slovakia 
#| fig-cap: Residuals Slovakia
lm_residuals_plot(Slovakia2010)
```
