...
Transport <- read_csv('transport.csv',show_col_types = FALSE )%>%
  filter(unit=="Kilometre" & tra_infr== "MWAY" & TIME_PERIOD ==2010 &(grepl('DK..', geo) | grepl('FR..', geo) | grepl('HU..', geo)| grepl('PT..', geo) | grepl('SK..', geo))) %>%
   select(unit,tra_infr, geo, TIME_PERIOD,OBS_VALUE) %>%
  mutate(Road=OBS_VALUE, Year2=TIME_PERIOD, Region2=geo) %>%
  select(Region2, Year2, Road)
```
...
filter(unit=="YEAR" & sex == "Total" & (grepl('DK..', geo) | grepl('FR..', geo) | grepl('HU..', geo) | grepl('PT..', geo) | grepl('SK..', geo))) %>%
   mutate(lea = OBS_VALUE, Year2 = TIME_PERIOD, Region2=geo) %>%
...
Per_capita2 <- gini_n2 %>%
  ...
  filter(Year=="2010" & unit =='MIO_EUR' & (grepl('DK..', Region) |     grepl('FR..', Region) | grepl('HU..', Region)|     grepl('PT..', Region) | grepl('SK..', Region))) %>%
  # select only columns we need
  select(Year2, Region2, GDP, unit) %>%
  left_join(Pop, by=c("Year","Region"),
            suffix = c("","Population")) %>%
   # only rows with TOTAL and T from the population data
  filter(age=='TOTAL' & sex=='T') %>%
  mutate(gdp_nuts2 =  GDP / Population* 1000000) %>%
  mutate(nuts_id2 = Regio_id) %>%
  select(Year2, nuts_id2, gdp_nuts2)
# join in our data with new variables
Year2010 <- gini_n2 %>%
  left_join(Education, by = c("Year", "nuts_id2")) %>%
  left_join(Rransport, by = c("Year", "nuts_id2")) %>%
  left_join(Lifexp, by = c("Year", "nuts_id2")) %>%
  left_join(percapita2, by = c("Year", "nuts_id2")) %>%
  # level of economic development = log(gdp per capita)
  mutate(log_gdppc = log(gdp_nuts2)) %>%
  # create column to easily filter by country
  mutate(nuts_id0 = substr(nuts_id2,1,2)) %>%
  filter(Year2==2010)
Year2010 %>%
  st(.)
