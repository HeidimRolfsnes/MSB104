...
The gini value goes from 0 to 1. Where 0 is a perfect equality and 1 is unequality. As we can see in the summary there are gini.... 
When the gini is 0, it is understandable to think there is missing som data for 2010. Further we will use filter to take away the gini´s who are zero. 
...
lm_function_plot <- function(lmdata){
  lm_gdppc <- lm(gini ~ gdppc, data = lmdata)
  b1_gdppc <- lm_gdppc$coefficients[1]
  b2_gdppc <- lm_gdppc$coefficients[2]

  # create plot for l.reg. model
  lmdata %>%
    ggplot(aes(x =gdppc, y=Gini, fill=nuts2, color=nuts2, label=nuts2)) +
    geom_point(lwd = .8) +
...
lm_residuals_plot <- function(lmdata){
  ...
  # create plot for l.reg. model
  lmdata %>%
    ggplot(aes(x =gdppc, y=gini, fill=nuts2, color=nuts2)) +
    geom_point(lwd = .8) +
    geom_segment(aes(xend=gdppc,yend=predicted)) +
    geom_abline(intercept = b1_gdppc, slope = b2_gdppc, color="red", size=0.8) + 
    labs(x = "(GDP per capita)", y = "Gini")
}
```
...
```
In this ggplot we look at the relationship between Gini and GDP per capita in 2010. We can see that very few of the points hit the line when we look at all the countries together.
...
when we look in the table above we can see the impact regional development (X) has on regional inequality (Y). this means how much an increase in X will mean for an increase in Y. Portugal is the only country that has a negative impact. We will take a closer look at this country by country. 


### Discuss the goodness of fit of our estiamted model 

When we look at the simple regression that we have done, it is R2 that can help us explain whether the variables have any relationship with each other. R2 tells us how much spread we have in the independent variable. The value in R2 can lie between 1 and 0, where R2 is 1 it tells us that the independent variable has all the influence on the dependent variable. if it is 0, the independent variable has no influence on the dependent variable. (Forskningsmetode - s.345)
 We can see that Denmark and France have a high R2 value, Denamrk with 0,813 and France with 0,814. Denmark only has 4 observations, which tells us that we cannot completely trust this result. France, on the other hand, has 22 observations, the more observations the better the result.
 when we look at all the countries together, we see that both R2 is only 0.122, we can see that in the context of
...
#| echo = FALSE
mlrdata <- Denmark2010
lm.7 = lm(Gini2 ~ Edu , mlrdata) 
lm.8 = lm(Gini2 ~  Road, mlrdata)
lm.9 = lm(Gini2 ~ Edu + Road, mlrdata)
lm.10 = lm(Gini2 ~  lea, mlrdata)
lm.11 = lm(Gini2 ~ Edu + lea, mlrdata)
lm.12 = lm(Gini2 ~ Road + lea, mlrdata)
...
#Gen Table
  ...
  statistics = c(N = "nobs", R2 = "r.squared"),
  coefs = c(Education = "Edu", Motorway = "Road", Lifeexp = "lea", const. = "(Intercept)"),
  note = "Note: {stars} T statistics in brackets."
  )
...
lm.7 = lm(Gini2 ~ Edu , mlrdata) 
lm.8 = lm(Gini2 ~  lea, mlrdata)
lm.9 = lm(Gini2 ~ Edu + lea, mlrdata)
...
#Gen Table
  ...
  statistics = c(N = "nobs", R2 = "r.squared"),
  coefs = c(Education = "Edu", Lifeexp = "lea", const. = "(Intercept)"),
  note = "Note: {stars} T statistics in brackets."
  )
...
Road har ingen tall n/A observasjoner 
...
OLS the OLS estimator chooses the parameters beta1 and beta2 that minimize the residual sum of squares (RSS)
