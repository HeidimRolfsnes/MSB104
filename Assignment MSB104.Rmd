...
We made a new data set called BNP2 (Norwegian word for GDP and a number). In this data set we will gather the information we need from the other two data sets and combine them into one data set. This will do it easier in our ongoing research.  
...
It is not easy to draw any conclusion from this summary. We see that there is a very big difference in GDP as the lowest value is 750 while the highest is 251 623. We can also see that the maximum value is high compared to the other values, as the 3rd quartile is only 18 630. The same we can se in Population and Per_capita. 

To get a better picture of the different countries and the opportunity to see if there are regions that stand out. We make summaries per country, because then we have the opportunity to exclude regions that stand out in futher assignments. We will also look at a spesific year in each country to compare results with.  
...
Lets start with Denmark and then we take the countries alphabetically.

##Denmark

Denmark is also a small country so there are not that many NUTS2 regions.  
...
Denmark are missing observations for some of the years, so there are less data to be collected about them.
